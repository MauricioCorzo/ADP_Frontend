...
const Paciente = ({paciente}) => {
    ...
    const formatearFecha = (fecha) => {    // formatea la fecha, es nativo de javascript, no necesita librerias y no modifica la variable de fecha
  ...
  return (
    <div className='mx-5 my-10 bg-white shadow-md px-5 py-10 rounded-xl'>
        ...
        <div className='flex justify-between my-5'>
            <button type='button' onClick={() => setEdicion(paciente)} className='py-2 px-10 bg-indigo-600 hover:bg-indigo-700 text-white uppercase font-bold rounded-lg'>Editar</button>
            <button type='button' onClick={() => eliminarPaciente(_id)} className='py-2 px-10 bg-red-600 hover:bg-red-700 text-white uppercase font-bold rounded-lg'>Eliminar</button>
